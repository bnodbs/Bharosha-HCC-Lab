...
export default async function TestsMasterPage({
  ...
  searchParams: { search?: string; category?: string; showInactive?: string };
}) {
  ...
  const tests = await prisma.test.findMany({
    ...
    include: {
      _count: {
        select: { parameters: { where: !showInactive ? { isActive: true } : undefined } }
      }
    }
  });
      ...
      <div className="bg-white rounded-lg shadow p-6 border">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {tests.map((test) => (
                <tr key={test.id} className={`border-b hover:bg-gray-50 transition-colors ${!test.isActive ? 'opacity-60 bg-gray-50' : ''}`}>
                  <td className="p-3 font-medium text-gray-900">
                    {test.code}
                    {!test.isActive && <span className="ml-2 text-[10px] bg-red-100 text-red-700 px-1 py-0.5 rounded">INACTIVE</span>}
                  </td>
